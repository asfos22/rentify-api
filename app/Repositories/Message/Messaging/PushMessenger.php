...
class PushMessenger implements MessengerInterface
{
    ...
    public function send(MessageInterface...$messages): bool
    {
       ...
       // $cloudMessages = [];
        ...
        foreach ($messages as $message) {
            ...
            //dump("MSG SERVICE PUSH ", $messages);
            //exit();
            ...
            try {
                ...
                $this->fcmInterface->sendBatchNotification($pushToken, [
                    'topicName' => 'test',
    ...
    {
        return $this->repository->fetchForUser($user->getId(), 0, 0/*$filter->getLimit(), $filter->getOffset(), $filter->getSelections(), $filter->getFilters(), $filter->getOrderings()*/);
    }
